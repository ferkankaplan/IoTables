...
- Tenant creation success shows tenant identity, tenant host, lifecycle state, starter template state, and admin bootstrap state.
...
- `provisioning_failed` and `suspended` states need stronger visual priority than normal `active` state.
- Tenant health summary must not visually imply unsupported runtime control.

### Copy Requirements

- Use "tenant unavailable", "setup failed", and "provisioning" language.
- Do not expose per-tenant DNS controls in the current release.

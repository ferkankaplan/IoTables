...
| `otp_messaging.send_otp` | Identity and Access, TenantApp/CashierApp setup surfaces, worker | challengeId | Challenge not expired/verified/locked; setup flow owns challenge; max 3 sends in the current release; provider payload redacted | Insert `message_deliveries`; enqueue Reliable Side Effects outbox work; provider send happens only after commit | Delivery attempt state |
...
| --- | --- | --- | --- | --- |
| `otp_messaging.get_challenge_state` | TenantApp, CashierApp, Identity and Access | challengeId | User/session must match setup flow; do not expose OTP code | Expiry, retry count, send count, verification state |
...
| `target_changed_not_applied` | Tenant GSM changed after challenge creation; existing challenge still uses original target. |

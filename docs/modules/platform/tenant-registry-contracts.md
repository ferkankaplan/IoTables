...
| --- | --- | --- | --- | --- | --- |
| `tenant_registry.register_identity` | Provisioning | name, subdomain, gsmNumber, sector, optional capacity/address | Platform Owner already authorized by Provisioning; tenant-creation OTP verified; name/subdomain/GSM required; subdomain normalized and unique; GSM unique | Insert tenant as `provisioning`; unique lower(subdomain) and unique GSM prevent duplicate tenant identity | Tenant identity with `tenantId` and `status = provisioning` |
| `tenant_registry.update_profile` | PlatformApp | tenantId, editable platform-owned profile fields | Platform Owner only; name/subdomain/lifecycle immutable; GSM remains platform-owned identity GSM | Single tenant row update; GSM changes require audit | Updated tenant profile |
...
| `tenant_registry.get_profile` | PlatformApp, TenantApp | tenantId | Platform can read all; TenantApp own tenant only | Tenant identity/profile without runtime data; TenantApp receives read-only identity GSM if exposed in protected admin UI |

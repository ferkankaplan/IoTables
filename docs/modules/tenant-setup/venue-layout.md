...
| Table | active -> disabled | tenant, id, hall, name, displayOrder, enabled | Table belongs to exactly one hall; only one active TableSession per tenant/table is allowed by Settlement; current release uses ordered grid, not visual coordinates | Disable instead of hard-delete when sessions/orders/display credentials exist |
